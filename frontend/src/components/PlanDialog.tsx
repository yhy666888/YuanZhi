...
import { api, type Plan, type PlanType, type Priority } from "../api";
import { DatePicker, DateTimePicker } from "./DateTimePicker";

export function PlanDialog({ type, defaultDate, plan, onClose, onSaved }: { type: PlanType; defaultDate: string; plan?: Plan; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try { if (plan) await api.updatePlan(plan.id, values); else await api.createPlan({ plan_type: type, ...values, repeat_type: type === "daily" ? repeatType : "none", repeat_interval: Number(form.get("repeat_interval") || 1), repeat_weekdays: form.getAll("repeat_weekdays").map(Number) }); await onSaved(); } catch (err) { setError(err instanceof Error ? err.message : "保存计划失败"); } finally { setSaving(false); }
  };
